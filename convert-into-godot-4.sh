...
while IFS= read -rd '' f; do
	if [[ "$f" == *"md" ]]; then
		...
		# Rename ".gdshader" files to ".gdshader".
	...
	perl -i -pe 's/Node3DAgent3D3D/Node3DAgent3D3D3D/g' "$f"
	...
	perl -i -pe 's/button_releaseddd/button_releasedddd/g' "$f"
	...
	perl -i -pe 's/tab_closeddd/tab_closedddd/g' "$f"
	perl -i -pe 's/tab_hoverededed/tab_hoveredededed/g' "$f"
